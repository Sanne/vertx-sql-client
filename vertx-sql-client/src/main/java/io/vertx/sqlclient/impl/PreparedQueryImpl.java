...
class PreparedQueryImpl<T, R extends SqlResult<T>> implements PreparedQuery<R> {
  ...
  static PreparedQuery<RowSet<Row>> create(Connection conn, ContextInternal context, PreparedStatement ps, boolean autoCommit) {
    SqlResultBuilder<RowSet<Row>, RowSetImpl<Row>, RowSet<Row>> builder = new SqlResultBuilder<>(conn, ps, autoCommit, RowSetImpl.FACTORY, RowSetImpl.COLLECTOR);
  ...
  public <R2> PreparedQuery<SqlResult<R2>> collecting(Collector<Row, ?, R2> collector) {
    SqlResultBuilder<R2, SqlResultImpl<R2>, SqlResult<R2>> builder = new SqlResultBuilder<>(conn, ps, autoCommit, SqlResultImpl::new, collector);
    return new PreparedQueryImpl<>(conn, context, ps, autoCommit, builder);
  }

  @Override
  public <U> PreparedQuery<RowSet<U>> mapping(Function<Row, U> mapper) {
    SqlResultBuilder<RowSet<U>, RowSetImpl<U>, RowSet<U>> builder = new SqlResultBuilder<>(conn, ps, autoCommit, RowSetImpl.factory(), RowSetImpl.collector(mapper));
    return new PreparedQueryImpl<>(conn, context, ps, autoCommit, builder);
  }
