...
public class CursorImpl implements Cursor {
  ...
  public synchronized Future<RowSet<Row>> read(int count) {
    ...
    SqlResultBuilder<RowSet<Row>, RowSetImpl<Row>, RowSet<Row>> builder = new SqlResultBuilder<>(ps.conn, ps.ps, ps.autoCommit, RowSetImpl.FACTORY, RowSetImpl.COLLECTOR);
    if (id == null) {
      id = UUID.randomUUID().toString();
      result = builder.execute(params, count, id, false, promise);
    } else if (result.isSuspended()) {
      result = builder.execute(params, count, id, true, promise);
    } else {
